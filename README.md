...
Целият списък с думи, можете да намерите в `collatiō.txt` (с макронизация) и `collatiō-simplex.txt` (без макронизация).
...
1. Разделяне на прилагателните на изключителни, числителни и двете стандартни групи
2. Да се измисли най-адекватния метод за изписване на записите на прилагателните
3. Разделяне на наречията по вид (за място, за време, за начин, за сравнение и т.н.)
4. Автоматично генериране на другите две форми за глаголи от първо и четвърто склонения
5. Разделяне на местоименията на въпросителни, определени и неопределени
6. Вместо (+acc) -> (), (+dat) -> [], (+abl) -> {}, (+gen) -> <>
